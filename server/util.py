...
def dict2xls(tabinfo_list, xlspath):
  ...
  for ti, tabinfo in enumerate(tabinfo_list):
    ...
    if row_num == 1:
      colsort = np.argsort(np.array(max_col_pos)[:, 0])
      for ci in colsort:
        ws.cell(row=1, column=ci + 1, value=tabinfo['row_1'][ci]['text'])
    else:
      # 获得列细分pos和行细分pos
      base_row_pos = []
      # row_diff = []
      for rkey in tabinfo:
        ...
        for i, colinfo in enumerate(tabinfo[rkey]):
          row_h.append(
            (colinfo['cell_pos'][5] - colinfo['cell_pos'][3] + colinfo['cell_pos'][7] - colinfo['cell_pos'][1]) / 2)
      ...
      for rkey in tabinfo:
        ...
        for col_i, colinfo in enumerate(tabinfo[rkey]):
          ...
          if colinfo['text'] != '' and colinfo['text'] != ' ':
            try:
              if ws.cell(row=ri, column=ci).value:
                ws.cell(row=ri, column=ci, value=ws.cell(row=ri, column=ci).value + colinfo['text'])
              else:
                ws.cell(row=ri, column=ci, value=colinfo['text'])
            except:
              if len(merged_cell_ranges) > 0:
          ...
          if (start_row != end_row or start_column != end_column) and (
